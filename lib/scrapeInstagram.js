...
async function scrapeInstagram(username, { count = 6 } = {}) {
  ...
  try {
    ...
    try {
      ...
      // Diagnóstico: si el grid nunca aparece, es más útil saber QUÉ nos
      // sirvió Instagram (login wall, checkpoint, error...) que solo "timeout".
      ...
      const info = diagnosis
        ? ` | title="${diagnosis.title}" url="${diagnosis.url}" body="${(diagnosis.bodyText || '').replace(/\s+/g, ' ').trim()}"`
        : ' | no se pudo inspeccionar la página';
      throw new Error(`${waitErr.message}${info}`);
    }
    ...
    for (const r of raw) {
      ...
      let video = null;
      try {
        await page.goto(r.url, { waitUntil: 'networkidle2', timeout: 40000 });
